...
{
  if(myRank == 0)
    cout << "== read mesh file (" << fileName << ")" << endl;
  ...
  while(meshfile.good()){
    ...
    if(line.compare("$Elements") == 0){
      ...
      for(int iTri=0; iTri<mesh.nbOfTri; iTri++){
        ...
        numPartMsh = (numPartMsh > elemPart) ? numPartMsh : elemPart;
  ...
  if(myRank == 0){
    cout << "   -> " << numPartMsh << " subdomains in the mesh" << endl;
    cout << "   -> " << nbTasks << " MPI processes" << endl;
  }
  meshfile.close();
}
